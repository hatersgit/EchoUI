...
function createTransmogWindow()
    ...
    TransmogWindow.header:SetScript(
        "OnMouseDown",
        function()
    ...
    TransmogWindow.header.close =
        CreateFrame("BUTTON", "InstallCloseButton", TransmogWindow.header, "UIPanelCloseButton")
    ...
    TransmogWindow.header.close:SetScript(
        "OnClick",
        function()
            TransmogWindow:Hide()
        end
    ...
    TransmogWindow.modelhub.accept =
        CreateFrame("BUTTON", TransmogWindow.modelhub.accept, TransmogWindow.modelhub, "UIPanelButtonTemplate")
    ...
    TransmogWindow.modelhub.accept:SetScript(
        "OnClick",
        function()
    ...
    TransmogWindow.collection:SetSize(
        tmogsettings.width - tmogsettings.width / 2.5,
        tmogsettings.height - tmogsettings.headerheight
    )
    ...
    for i = 1, rows do
        ...
        for j = 1, columns do
            TransmogWindow.collection.buttons[i][j] =
                CreateFrame("BUTTON", TransmogWindow.collection.buttons[i][j], TransmogWindow.collection)
            ...
            TransmogWindow.collection.buttons[i][j]:SetScript(
                "OnEnter",
                function()
    ...
    TransmogWindow.collection.pagenext =
        CreateFrame("BUTTON", TransmogWindow.collection.pagenext, TransmogWindow.collection, "UIPanelButtonTemplate")
    ...
    TransmogWindow.collection.pagenext:SetScript(
        "OnClick",
        function()
    ...
    TransmogWindow.collection.pageback =
        CreateFrame("BUTTON", TransmogWindow.collection.pageback, TransmogWindow.collection, "UIPanelButtonTemplate")
    ...
    TransmogWindow.collection.pageback:SetScript(
        "OnClick",
        function()
...
function UpdateCollectionView()
    ...
    for _, data in ipairs(page) do
        ...
        TransmogWindow.collection.buttons[row][column]:HookScript(
            "OnEnter",
            function()
        ...
        TransmogWindow.collection.buttons[row][column]:SetScript(
            "OnLeave",
            function()
...
function SetPageNumber(number)
    ...
    if not TransmogWindow.collection.pagenumber then
        TransmogWindow.collection.pagenumber =
            TransmogWindow.collection:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
        TransmogWindow.collection.pagenumber:SetPoint(
            "BOTTOM",
            TransmogWindow.collection,
            "BOTTOM",
            0,
            .2 * tmogsettings.headerheight
        )
...
function InitSetDropdown()
    TransmogWindow.modelhub.setDropdown =
        CreateFrame("Frame", "setDropdown", TransmogWindow.modelhub, "UIDropDownMenuTemplate")
    ...
    UIDropDownMenu_Initialize(
        TransmogWindow.modelhub.setDropdown,
        function(self, _, _) --self,level,menuList
            for _, item in ipairs(self.sets) do
                local info = UIDropDownMenu_CreateInfo()
                info.text = item.name
                info.value = item.setid
                info.func = function(_) --self
                    PENDING_CHANGES = {}
...
function SetSlotIcon(slot, icon, itemid)
    ...
    if TransmogWindow.modelhub.slots[slot].id ~= TransmogWindow.modelhub.slots[slot].appearance then
        ...
        TransmogWindow.modelhub.slots[slot].reset:SetScript(
            "OnClick",
            function()
...
StaticPopupDialogs["REQUEST_XMOG_SET_NAME"] = {
    ...
    OnAccept = function(self, _, _) --data,data2
        local text = self.editBox:GetText()
        AcceptSetName(text)
    end,
    EditBoxOnEscapePressed = function(self, _) --input
        self:GetParent():Hide()
    end,
    EditBoxOnEnterPressed = function(self, _) --input
