...
function createWorldTierSelectWindow()
    ...
    WorldTier.header.close:SetScript(
        "OnClick",
        function()
            WorldTier:Hide()
        end
    ...
    WorldTier.body.buttonBox.Start =
        CreateEchosButton(
        WorldTier.body.buttonBox.Start,
        WorldTier.body.buttonBox,
        WorldTier.body.buttonBox:GetWidth(),
        WorldTier.body.buttonBox:GetHeight() / 7,
        "Begin",
        12
    )
    ...
    WorldTier.body.Prestige.text:SetText(
        "\124cffBC961CPrestiging\124r now will reset your character back to level 1 and send you back to your starting zone. If you had a complete set of perks from a previous run they will be retained and shown back to you randomly when receiving a perk roll.\n\nShould you wish to abandon your prestige run you may do so by teleporting back to the hub and prestiging again, however \124cffFF0000YOU WILL LOSE YOUR SAVED PERKS\124r.\n\n\124cffBC961CTo unlock higher world tiers you must complete a prestige within the preceding world tier.\124r"
    )

    SubscribeToForgeTopic(
        ForgeTopic.SHOW_WORLD_TIER_SELECT,
        function(msg)
            if msg == "0" then
                WorldTier:Hide()
            else
...
function drawButtons()
    ...
    for i = 1, BASE.UNLOCKED_MAX, 1 do
        if not WorldTier.body.buttonBox.buttons[i] then
            WorldTier.body.buttonBox.buttons[i] =
                CreateEchosButton(
                WorldTier.body.buttonBox.buttons[i],
                WorldTier.body.buttonBox,
                WorldTier.body.buttonBox:GetWidth(),
            ...
            WorldTier.body.buttonBox.buttons[i]:SetScript(
                "OnClick",
                function()
