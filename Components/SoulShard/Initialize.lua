    ...
    function(msg)
        ...
        for i, shard in ipairs(listOfObjects) do
            ...
            if s then
                ...
                ActiveShardWindow.Shards[i]:SetScript(
                    "OnEnter",
                    function()
...
SubscribeToForgeTopic(
    ...
    function(msg)
        ...
        for _, shard in ipairs(listOfObjects) do
            ...
            shardFrame:SetPoint(
                "TOPLEFT",
                baseX + padding * (itt - 1) + (itt - 1) * iconsize,
                -baseX - 1 * (padding * (row - 1) + (row - 1) * iconsize)
            )

            shardFrame:SetScript(
                "OnEnter",
                function()
            ...
            shardFrame:SetScript(
                ...
                function()
                    if SoulShard.clicked then
                        ...
                        if prevShard > 0 then
                            SoulShard.ShardToFrame[prevShard]:SetBackdropBorderColor(
                                RankToQualityColor(SoulShard.Shards[prevShard].rank)
                            )
                        end
