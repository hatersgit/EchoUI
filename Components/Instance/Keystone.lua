...
function createKeySetupWindow()
    ...
    KeySetupWindow.header:SetScript(
        "OnMouseDown",
        function()
    ...
    KeySetupWindow.header.close =
        CreateFrame("BUTTON", "InstallCloseButton", KeySetupWindow.header, "UIPanelCloseButton")
    ...
    KeySetupWindow.header.close:SetScript(
        "OnClick",
        function()
            KeySetupWindow:Hide()
        end
    ...
    KeySetupWindow.body.affixes.title:SetPoint(
        "TOP",
        KeySetupWindow.body.affixes,
        "TOP",
        0,
        -tmogsettings.headerheight / 2
    )
    ...
    KeySetupWindow.body.time.bronze:SetPoint(
        "CENTER",
        KeySetupWindow.body.time,
        "LEFT",
        KeySetupWindow:GetWidth() / 5,
        tmogsettings.headerheight / 2
    )
    ...
    KeySetupWindow.body.time.silver:SetPoint(
        "CENTER",
        KeySetupWindow.body.time,
        "CENTER",
        0,
    ...
    KeySetupWindow.body.time.gold:SetPoint(
        "CENTER",
        KeySetupWindow.body.time,
        "RIGHT",
        -KeySetupWindow:GetWidth() / 5,
    ...
    KeySetupWindow.body.time.start:SetScript(
        "OnClick",
        function()
            local out = ""
            local delim = ""
...
local function initAffixIconsForTier(tray, affixlist, iconSize, target)
    for i = 1, #affixlist do
        ...
        local _, _, icon = GetSpellInfo(affix.spell)
...
SubscribeToForgeTopic(
    ForgeTopic.MYTHIC_SET_AFFIXES_AND_START,
    function(msg)
        --print(msg)
        local split = ForgeSplit("*", msg)
        DG_COMPLETE = false
...
SubscribeToForgeTopic(
    ForgeTopic.MYTHIC_GET_AFFIXES_LIST,
    function(msg)
        --print(msg)
...
SubscribeToForgeTopic(
    ForgeTopic.MYTHIC_UPDATE_CRITERIA,
    function(msg)
        --print(msg)
        local minionStr = "Minions: "
        local INSTANCEStr = ""
        for _, entry in ipairs(DeserializeMessage(DeserializerDefinitions.GET_MYTHIC_OBJS, msg)) do
            if entry.ID == "0" then
...
SubscribeToForgeTopic(
    ForgeTopic.MYTHIC_UPDATE_DEATHS,
    function(msg)
...
SubscribeToForgeTopic(
    ForgeTopic.MYTHIC_UPDATE_TIMER,
    function(msg)
        --print(msg)
        CURRENT_TIMER = tonumber(msg)
        local t = 1
        KeyProgress:SetScript(
            "OnUpdate",
            function(_, elapsed)
...
SubscribeToForgeTopic(
    ForgeTopic.MYTHIC_KEY_COMPLETED,
    function(msg)
        DG_COMPLETE = true
        -- KeyProgress:SetScript(
        --     "OnUpdate",
